---
title: Self-hosting MoneySystem Free Using Supabase
description: Today, we're pleased to share that MoneySystem can now be self-hosted. This article will delve into what's behind the scenes and guide you on how to self-host MoneySystem using Supabase.
author: Nathan Foo
author_image_url: /images/blog/authors/nathan.png
author_url: https://github.com/MoneySystem
image: /images/blog/self-hosting-appflowy-free-using-supabase/cover.png
thumb: /images/blog/self-hosting-appflowy-free-using-supabase/cover.png
tags:
  - Tech Design
  - How-tos
categories:
...
<Caption content="MoneySystem Self-hosted" />
Today, we're pleased to share that MoneySystem can now be self-hosted. This article will delve into what's behind the scenes and guide you on how to self-host MoneySystem using Supabase.

MoneySystem is a privacy-first, open source workspace for your notes, wikis, projects, and more. You are in charge of your data and customizations, with no vendor lock-in.
...
## Introducing Cloud Enabled MoneySystem

In order to implement cloud features such as user sign-ins and data storage, we've partnered with [Supabase](https://supabase.com/?ref=blog.appflowy.io), an open-source alternative to [Firebase](https://firebase.google.com/?ref=blog.appflowy.io).

With a Supabase project, transforming MoneySystem into a self-hosted cloud app is straightforward. By the end of this article, you'll be primed to launch your very own MoneySystem cloud application with ease. Furthermore these steps are similar to other cloud services, such as `Appwrite` or `Firebase`, should you want to use those services for an MoneySystem cloud implementation.

Let's explore how MoneySystem uses Supabase to implement the cloud features.
...
We can leverage Supabase's authentication service to implement MoneySystem's authentication system. After following the instructions the [Supabase auth documentation](https://supabase.com/docs/guides/auth?ref=blog.appflowy.io), we successfully set up the authentication service.

Let's explore how we integrated Supabase authentication into MoneySystem.

### Auth Code Walkthrough

Inetgrating MoneySystem with Supabae requires the [supabase\_flutter](https://pub.dev/packages/supabase_flutter?ref=blog.appflowy.io) Flutter package.
...
1.  The user clicks the login button in MoneySystem.
2.  MoneySystem requests Supabase to start the authentication process.
...
6.  Supabase verifies the credentials and sends a user token back to MoneySystem.
7.  MoneySystem updates to show the user they're logged in.

<Img src="/images/blog/self-hosting-appflowy-free-using-supabase/img-1.png" alt="MoneySystem authentication flow" />

By clicking a login button for Google, users are directed to a web browser to finalize the authentication process. Once authenticated, they are redirected back to MoneySystem.

We will support other social logins later, including GitHub, Discord, Slack, and more. The process for each is quite similar

<Img src="/images/blog/self-hosting-appflowy-free-using-supabase/img-2.png" alt="MoneySystem login flow" />

## Data Storage

### Architectural Design

One of the prerequisites for MoneySystem Data Storage is support for multiple cloud services.
...
<Img src="/images/blog/self-hosting-appflowy-free-using-supabase/img-3.png" alt="MoneySystem cloud service architecture" />

Using the `AppFlowyServer` interface we can easily transition between various cloud services. For instance, moving from [Supabase](https://supabase.com/?ref=blog.appflowy.io) to [Firebase](https://firebase.google.com/?ref=blog.appflowy.io) or [Appwrite](https://appwrite.io/?ref=blog.appflowy.io) is merely a matter of changing the implementation of the `AppFlowyServer` interface.
...
The `DatabaseCloudService` represents a set of standardized operations focused on collaborative objects within a `MoneySystem` database.
...
Upon integrating Supabase authentication, we discovered it inherently offers a PostgreSQL database. This advantage implies that individual users self-hosting MoneySystem on Supabase won't require separate data storage setups.

Each Supabase project is pre-equipped with a dedicated PostgreSQL database, which we leverage to store MoneySystem's data.
...
1.  User profiles
2.  User settings
3.  Data related to documents and databases generated within MoneySystem
4.  Relationships between users and their associated workspaces

Here's what our schema looks like:

<Img src="/images/blog/self-hosting-appflowy-free-using-supabase/img-4.png" alt="MoneySystem database schema" />
...
We've developed a [tool](https://github.com/MoneySystem-IO/MoneySystem-Supabase?ref=blog.appflowy.io) designed to set up the database tables, triggers, and functions within a Supabase PostgreSQL database. With this tool, a single command can configure your database. For a detailed guide, please consult [this documentation](https://github.com/MoneySystem-IO/MoneySystem-Supabase/tree/main/postgres?ref=blog.appflowy.io).

#### DB Code Walkthrough

MoneySystem's backend is developed in Rust, which requires a Rust crate to interact seamlessly with the Postgres database.

Thankfully, the community has provided [postgrest-rs](https://github.com/supabase-community/postgrest-rs?ref=blog.appflowy.io), a crate specifically designed to handle Supabase's Postgres database features. We've defined a struct named [SupabaseServer](https://github.com/MoneySystem-IO/MoneySystem/blob/main/frontend/rust-lib/flowy-server/src/supabase/server.rs?ref=blog.appflowy.io) that implements the `AppFlowyServer` trait.
...
With Supabase's [realtime](https://supabase.com/docs/guides/realtime?ref=blog.appflowy.io) service, we can monitor changes to specific tables.
...
<Img src="/images/blog/self-hosting-appflowy-free-using-supabase/img-5.png" alt="MoneySystem monitoring" />
...
File storage in MoneySystem is still under development.

Once implemented, users will be able to store images, videos, documents, and various other file types.

## How to Self-host MoneySystem with Supabase

We've tried to make the process of self-hosting MoneySystem as straightforward as possible. Please follow the steps below to guide you through the process:

1.  **Set up a Supabase Project**: Start by creating a new Supabase project. For detailed instructions, refer to [this documentation](https://supabase.com/docs/guides/getting-started/tutorials/with-flutter?ref=blog.appflowy.io).
2.  **Configure Your Postgres Database**: For guidance on setting up your Postgres database, consult this [guide](https://github.com/MoneySystem-IO/MoneySystem-Supabase/tree/main/postgres?ref=blog.appflowy.io).
3.  **Fork the Repository**: Fork the [Self-hosting-template](https://github.com/MoneySystem-IO/Self-hosting-template?ref=blog.appflowy.io) repository on GitHub.
...
<Img src="/images/blog/self-hosting-appflowy-free-using-supabase/img-6.png" alt="Environment secrets" />

<Img src="/images/blog/self-hosting-appflowy-free-using-supabase/img-7.png" alt="Environment secrets" />
...
<Img src="/images/blog/self-hosting-appflowy-free-using-supabase/img-8.png" alt="Deploying with a new tag" />

## Questionnaire

Thanks for reading this article. If you have some time, please kindly take our [1-minute survey](https://tally.so/r/w2aVej?ref=blog.appflowy.io) to give us feedback and let us know what interests you the most.

If you have a suggestion or question for self-hosted solutions or any part of MoneySystem, you can [create an issue](https://github.com/MoneySystem-IO/MoneySystem/issues/new/choose?ref=blog.appflowy.io) on GitHub and we’ll get back to you as soon as possible. Also feel free to follow us @appflowy on [Twitter](https://twitter.com/appflowy?ref=blog.appflowy.io) or join our [Discord server](https://api.whatsapp.com/send?phone=5548988745520&text=Ol%C3%A1!%20Tenho%20interesse%20no%20MoneySystem?ref=blog.appflowy.io) to follow our latest development!