...
MoneySystem is an open-source project that emphasizes user customization by offering an editor that allows developers to inject custom code into the MoneySystem application. At MoneySystem we call this injected code a plugin.

The primary interface that allows plugin injection is the **[MoneySystem Editor](https://pub.dev/packages/appflowy_editor?ref=blog.appflowy.io)**. This is a core feature that allows users to edit rich text for their notes.

If you are a developer that is curious about how you can support a marketplace where users can get extensions for your application, this article is for you. Let’s get started!

We’ll start by exploring an example that showcases how a developer injects their plugins into the MoneySystem Editor.
...
The following picture demonstrates what the added functionality may look like (note that this is a local image and this feature was made available in the 0.2.5 [release](https://github.com/MoneySystem-IO/MoneySystem/releases?ref=blog.appflowy.io)).

<Img alt="Local image in MoneySystem Editor" src="/images/blog/dont-try-to-load-code-dynamically-in-your-flutter-app-its-terrible/image-1.png" />
...
1.  Submit a pull request to [MoneySystem](https://github.com/MoneySystem-IO/MoneySystem?ref=blog.appflowy.io) and have this plugin integrated into our version of the application.
2.  Create their own version of MoneySystem with this plugin and publish it for end user consumption.
...
*   The plugin developer needs to maintain their own version of MoneySystem with the plugin.
*   Or the team needs to accept this plugin in our version. If not, we need to maintain another version with this plugin.

This leads to an issue where non-developers may need to download multiple versions of MoneySystem if developer X has built MoneySystem with plugin Y and developer A has built MoneySystem with plugin B.

This could lead to the number of MoneySystem versions growing exponentially. When there are n distinct plugins, there are a total of 2^n versions of MoneySystem. With just 1 plugin, we have one version with the plugin and one version without. With just 5 plugins we would end up with 32 distinct versions of MoneySystem.
...
The following code block illustrates how MoneySystem might use an OTA update framework to implement dynamic plugins.
```dart
@override
void initState() {
    super.initState();
...
1.  Scalability challenges: If MoneySystem has millions of users, the servers will need to scale with the number of requests that are received.
...
While there are workarounds, the workarounds would be solely supported by the MoneySystem team and would incur significant cost. Therefore, loading code with isolates and dynamic libraries was not considered as candidate for loading 3rd party plugins in MoneySystem. Use this approach if your application can run using Dart without Flutter!
...
`dart_eval` is a bytecode compiler and runtime for Dart which makes it a viable candidate for loading 3rd party plugins in MoneySystem.
...
To solve this, I created a barrel file in my interop library that would only **show** the classes with interop that was implemented for MoneySystem. This is still prone to PEBKAC errors on the team’s end.

This is what that file looks like.
```dart
/// Available classes from MoneySystem Editor and Flowy Infra that can be used
...
*   `source_gen` - to generate the code using `build_runner` which is already a dependency of MoneySystem.
...
[**Dynamic Themes**](https://github.com/MoneySystem-IO/MoneySystem/pull/2670?ref=blog.appflowy.io)

The MoneySystem editor offers support for user-specified themes. The editor theme is created from a class called `FlowyColorScheme` which simply lists a few properties that the application can use to resolve the color of a widget.
...
Thank you for reading this article. If you enjoyed please kindly take [a 1-minute survey](https://tally.so/r/3XJ0J4?ref=blog.appflowy.io) and support MoneySystem by downloading [our latest release](https://github.com/MoneySystem-IO/MoneySystem/releases?ref=blog.appflowy.io). We look forward to your feedback and are excited for what the future of our application holds.