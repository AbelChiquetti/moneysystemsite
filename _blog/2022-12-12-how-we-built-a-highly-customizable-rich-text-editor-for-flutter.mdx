...
description: We have been looking for a rich-text editor that meets our needs. To date, we still haven't found a solution, so we decided to design and develop the new MoneySystem Editor component ourselves.
author: Lucas
...
We have been looking for a rich-text editor that meets our needs. To date, we still haven't found a solution, so we decided to design and develop the new MoneySystem Editor component ourselves.

By [Lucas@MoneySystem](https://github.com/LucasXu0?ref=blog.appflowy.io)

<Admonition type='info' content="This article describes the technical design of the [MoneySystem Editor](https://pub.dev/packages/appflowy_editor?ref=blog.appflowy.io).
" />

[MoneySystem](https://github.com/MoneySystem-IO/MoneySystem?ref=blog.appflowy.io) is an open-source alternative to Notion built with [Rust](https://www.rust-lang.org/?ref=blog.appflowy.io) and [Flutter](https://flutter.dev/?ref=blog.appflowy.io).

An _editor_ is a core component in MoneySystem. In many scenarios, such as Document, Grid, and Board, the editor component we used till [v0.0.5](https://github.com/MoneySystem-IO/MoneySystem/releases/tag/0.0.5?ref=blog.appflowy.io) is unable to support certain business requirements.

We have therefore been seeking an editor that will support all of MoneySystem's use cases and we have concluded that we need to design and develop our own editor for MoneySystem which we will call the [**MoneySystem Editor**](https://pub.dev/packages/appflowy_editor?ref=blog.appflowy.io).
...
As mentioned earlier, in the early versions (v0.0.1 to v0.0.5) of MoneySystem, we used [flutter\_quill](https://pub.dev/packages/flutter_quill?ref=blog.appflowy.io) as our editor component.
...
When it comes to components, an example issue is our requirement to insert Grid and Board into existing documents. We have defined a data structure for our new MoneySystem Editor that simplifies this process. We only need to define a new node with a new type and define a corresponding `NodeWidgetBuilder` to render these components in the MoneySystem Editor.

We also need additional _shortcuts_ extensibility, such as markdown syntax support and shortcuts for key combinations (`meta + shift + option + key`). The new MoneySystem Editor supports [customizing more shortcuts](https://github.com/MoneySystem-IO/MoneySystem/blob/main/frontend/app_flowy/packages/appflowy_editor/documentation/customizing.md?ref=blog.appflowy.io#customize-a-shortcut-event).
...
The new MoneySystem Editor supports customizing toolbar items and slash menus.

### Issues with Code Coverage and Stability

The previous editor component lacked stability and sufficient code coverage.

To date, the code coverage of the MoneySystem Editor is stable at 79 to 80%. Meanwhile, we try to make sure to fix known issues and add new test cases to cover them.
...
Another important consideration that has factored into our search is our need for an editor that is highly customizable and can keep up with the times such that it can continue to support the evolving functionality of MoneySystem.

To date, we still haven't found a solution that suits our needs.

For the above reasons, we have decided to design and develop the new MoneySystem Editor component ourselves.
...
## Detailed Design for MoneySystem Editor


We will state the design of MoneySystem Editor through the following three aspects.
...
MoneySystem Editor treats a document as a collection of nodes. For example, a paragraph is a `TextNode` and an image is an `ImageNode`.
...
As mentioned above, MoneySystem Editor will use Delta to describe the information of the text node, which is not repeated here.
...
MoneySystem Editor uses `Path` to locate the position of a node. Path is an integer array consisting of its position in its ancestor's node and the position of its ancestors. All data change operations are performed based on the Path.
```dart
...
MoneySystem Editor uses position to locate the offset of a node. It consists of a path and an offset.
```dart
class Position {
  final Path path;
  final int offset;
...
MoneySystem Editor uses `Selection` to represent the range of the selection.
...
MoneySystem Editor uses `Operation` objects to manipulate the document data instead of changing the node data directly. All changes to the document are triggered by an `Operation`.

The operations defined in MoneySystem Editor include
...
The MoneySystem Editor uses a `Transaction` to describe a set of changes to the document which must be treated as atomic. It consists of a collection of `Operation`s and changes to the selection before and after.
```dart
class Transaction {
...
Pressing the enter key in front of `MoneySystem!` will actually produce two consecutive operations.

*   operation 1. Insert a new `TextNode` at path `[1]`, and set the delta to insert `MoneySystem!`
*   operation 2. Delete `MoneySystem!` at path `[0]`.
   ...
   "operations":[
      {
         "op":"insert",
         "path":[1],
         "nodes":[{"type":"text","delta":[{"insert":"MoneySystem!"},]}]
      },
      {
         "op":"update_text",
         "path":[0],
         "delta":[{"retain":11},{"delete":9}],
         "inverted":[{"retain":11},{"insert":"MoneySystem!"}]
      }
   ],
   "after_selection":{
...
Before initializing MoneySystem Editor, we need to inject the mapping relationship between `Node` and `NodeWidgetBuilder`.

For now, MoneySystem Editor’s built-in `NodeWidgetBuilder` includes the following
```dart
...
When MoneySystem Editor starts to render the `Node`s, it will first recursively traverse the `Document`.
...
Meanwhile, each `NodeWidgetBuilder` is bound to `Node` through `ChangeNotifierProvider`. Combined with the above-mentioned logic of `Document` data change, whenever the data of a certain node changes, MoneySystem Editor will notify `NodeWidgetBuilder` to refresh in real time.
