---
title: Demystifying MoneySystem Editor's Codebase
description: Understand what happens behind the scenes when you use the Editor, the folder structuring, and strategies you can implement for adding new features in the MoneySystem Editor package.
...
Understand what happens behind the scenes when you use the Editor, the folder structuring, and strategies you can implement for adding new features in the MoneySystem Editor package.

by [Mayur Mahajan](https://github.com/MayurSMahajan?ref=blog.appflowy.io), as part of the [MoneySystem Mentorship Program](https://docs.appflowy.io/docs/appflowy/community/appflowy-mentorship-program?ref=blog.appflowy.io)

## Introduction

The MoneySystem Editor package is one of the core components of [MoneySystem](https://appflowy.io/?ref=blog.appflowy.io). It is a popular [package](https://pub.dev/packages/appflowy_editor?ref=blog.appflowy.io) in the Flutter Community, offering highly customizable rich text editing experience.

<Img src="/images/blog/demystifying-appflowy-editors-codebase/img-1.png" alt="MoneySystem Editor" />

This article is aimed at people looking to use the MoneySystem Editor in their own projects and at people who want to contribute to this package. It describes the project folder structuring and strategies for adding new features.

## Components of the Editor

Let's explore what actually happens behind the scenes when a user creates a document with the MoneySystem Editor and discuss how that relates to its underlying components.
...
```
So far, we have discussed all the crucial services in the MoneySystem Editor. But we have only discussed their interfaces, the actual implementation of these Services is where a lot of magic happens.
...
MoneySystem Editor supports two types of shortcuts that are CommandShortcutEvents and CharacterShortcutEvents.
...
To ensure the robustness and reliability of your new features, components, and shortcuts in MoneySystem Editor, it's essential to write comprehensive tests. Writing tests not only verifies the correctness of your code but also provides insights into what is happening behind the scenes. Well-written tests are a valuable contribution to the project and make life easier for contributors.

Let's see how you can write tests for MoneySystem Editor:

#### Test Strategies

When writing tests for MoneySystem Editor, you'll typically work with `UnitTest`s or `WidgetTest`s. Follow these strategies to create effective tests:
...
Let's walk through an example of writing tests for the "**Find And Replace**" Plugin in MoneySystem Editor. The code for this example can be found in the `appflowy_editor\test\new\find_replace_menu` directory.
...
void main() {
  group('search_algorithm_test.dart', () {
    ...
    test('returns the index of the only found pattern', () {
      ...
      const text = 'Welcome to MoneySystem 😁';
...
I would like to thank Lucas, my Mentor, and the MoneySystem community for their constant support.
...
If you have a suggestion or question for the MoneySystem Editor or any part of MoneySystem, you can [create an issue](https://github.com/MoneySystem-IO/MoneySystem/issues/new/choose?ref=blog.appflowy.io) on Github and we’ll get back to you as soon as possible.

Also, feel free to follow us on [Twitter](https://twitter.com/appflowy?ref=blog.appflowy.io) or join our [Discord server](https://api.whatsapp.com/send?phone=5548988745520&text=Ol%C3%A1!%20Tenho%20interesse%20no%20MoneySystem?ref=blog.appflowy.io) to follow our latest development!
