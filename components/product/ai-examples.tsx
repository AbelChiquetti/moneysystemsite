...
function AiExamples() {
  ...
  const tabOptions = useMemo(() => {
    return [
      ...
      { value: 'auto', label: 'Automação de Fluxo de Trabalho', icon: <AutomateIcon /> },
    ];
  }, []);
  ...
  return (
    <div ref={ref} className={'ai-examples'}>
      ...
      <TabPanel value={value} index={'write'}>
        <div className={'panel-title'}>
          Com base no seu faturamento, <span className={'text-[#6cb640]'}>saiba a previsão futura da sua loja</span> (Em breve)
        </div>
        <div className={'panel-image'}>
          <Image
            loading={'eager'}
      ...
      <TabPanel value={value} index={'auto'}>
        <div className={'panel-title'}>Reduza erros no trabalho! <span className={'text-[#6cb640]'}>Ferramentas com I.A</span> para automatizar tarefas
        </div>
        <div className={'panel-image'}>
          <Image
            loading={'eager'}
